...
# 根据论文设置的标准参数
user_num = 10  # 论文中使用的用户数量
edge_num = 10  # 论文中使用的边缘服务器数量

# 环境参数设置
env = Env(B=1e9, USER_NUM=user_num, EDGE_NUM=edge_num, F=6.3e7, f=1,
          Dn=np.random.uniform(2.7e4, 2.7e4, user_num),  # 传输数据大小
          Cn=np.random.uniform(1.08e6, 1.08e6, user_num),  # 处理数据大小
...
class Buffer:
    ...
    def update(
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # 双Q网络目标计算
            y1 = reward_batch + gamma * target_1_critic(
                [next_state_batch, target_actions], training=True
            )
            y2 = reward_batch + gamma * target_2_critic(
                [next_state_batch, target_actions], training=True
            )
            min_q_target = tf.minimum(y1, y2)

            # 当前Q值
            critic_1_value = critic_1_model([state_batch, action_batch], training=True)
            critic_2_value = critic_2_model([state_batch, action_batch], training=True)

            # Critic损失
            critic_1_loss = tf.math.reduce_mean(tf.math.square(min_q_target - critic_1_value))
            critic_2_loss = tf.math.reduce_mean(tf.math.square(min_q_target - critic_2_value))

        # 更新Critic网络
        critic_grad = tape.gradient(critic_1_loss, critic_1_model.trainable_variables)
        critic_optimizer.apply_gradients(
            zip(critic_grad, critic_1_model.trainable_variables)
        )

        critic_grad = tape.gradient(critic_2_loss, critic_2_model.trainable_variables)
        critic_optimizer.apply_gradients(
            zip(critic_grad, critic_2_model.trainable_variables)
        )

        # 延迟策略更新（TD3的另一个关键特性）
        if self.buffer_counter % 2 == 0:  # 每2步更新一次策略
            with tf.GradientTape() as tape:
                actions = actor_model(state_batch, training=True)
                critic_value = critic_1_model([state_batch, actions], training=True)
...
# TD3噪声参数设置
std_dev = 0.1  # 标准TD3噪声标准差
ou_noise = OUActionNoise(mean=np.zeros(1), std_deviation=float(std_dev) * np.ones(1))

actor_model = get_actor()
critic_1_model = get_critic()
critic_2_model = get_critic()

target_actor = get_actor()
target_1_critic = get_critic()
target_2_critic = get_critic()
# Making the weights equal initially
target_actor.set_weights(actor_model.get_weights())
target_1_critic.set_weights(critic_1_model.get_weights())
target_2_critic.set_weights(critic_2_model.get_weights())


# TD3算法超参数设置（根据论文和标准TD3参数）
critic_lr = 3e-4  # 标准TD3学习率
actor_lr = 3e-4   # 标准TD3学习率
...
# 经验回放缓冲区
buffer = Buffer(100000, 256)  # 更大的缓冲区，更大的批次大小
...
# TD3训练循环 - 根据论文参数设置
print("开始TD3训练...")
print(f"环境设置: {user_num}个用户, {edge_num}个边缘服务器")
print(f"训练参数: {total_episodes}个episode, 每个episode {3000}步")
print(f"网络架构: Actor(400-300), Critic(400-300)")
print(f"学习率: Actor={actor_lr}, Critic={critic_lr}")
print(f"缓冲区: {buffer.buffer_capacity}容量, {buffer.batch_size}批次大小")
...
for ep in range(total_episodes):
    ...
    the_best = 10000
    # 根据论文设置，每个episode 3000步
    for render in range(3000):
        ...
        update_target(target_1_critic.variables, critic_1_model.variables, tau)
        update_target(target_2_critic.variables, critic_2_model.variables, tau)

        prev_state = state
        the_best = min(the_best, cost)
    episodic_reward /= 3000  # 根据新的步数调整
